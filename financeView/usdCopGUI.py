...
def plot_last_month_exchange_rate():
    ...
    historical_data_api = f'https://api.exchangerate-api.com/v4/fluctuation/usd/cop/{start_date.strftime("%Y-%m-%d")}/{end_date.strftime("%Y-%m-%d")}'
    response = requests.get(historical_data_api)

    if response.status_code == 200:
        data = response.json()
        dates = [datetime.strptime(date, "%Y-%m-%d") for date in data['dates']]
        closing_rates = data['rates']['close']
